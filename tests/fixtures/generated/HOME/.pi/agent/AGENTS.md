...
`rm -rf`, `git reset --hard`, `git clean -f`, ลบ branch/tag แบบบังคับ, ลบ remote branch, destructive database, Docker prune/volume, global package install, shell startup file, git remote change, auth change, path นอก Trust Zone, staging/production deploy และ production database
...
- `gh pr merge`, `gh repo delete`, `gh gist`, `gh secret`, `gh auth token`
- credential file, keychain, production env (.env.production, .env.production.*, .env.prod, .env.prod.*)
