...
- sandbox ปฏิเสธการ enumerate process: `ps` ตอบ `operation not permitted: /bin/ps` (exit 127), `pgrep`/`pkill` ตอบ `Cannot get process list` (exit 3) — exit 3 แปลว่าอ่านไม่ได้ ไม่ใช่ไม่พบ process; เช็ค service ที่ฟัง port ด้วย `lsof -nP -iTCP -sTCP:LISTEN` ซึ่งทำงานใน sandbox ได้ ถ้าต้องดู process list จริง หรือ `kill` process นอก tree ของ sandbox (ตอบ `operation not permitted` เช่นกัน) ให้ user รันเองด้วย `!` prefix; test suite ที่จัดการ process tree เองจะแดงด้วย `spawnSync ps EPERM` ใน sandbox ให้รันนอก sandbox แทนการแก้โค้ด
