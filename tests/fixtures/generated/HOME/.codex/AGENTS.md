...
- token ของ `gh` อยู่ใน macOS keychain ซึ่ง seatbelt ของ Codex ปิดไว้ทุกโหมด (`~/Library/Keychains` deny + base policy) ดังนั้น `gh` และ `git push/pull/fetch/clone` กับ private repo หรือ remote ที่ต้อง auth ใช้ไม่ได้บน Codex: อาการคือ `HTTP 401: Requires authentication`, `The token in default is invalid` หรือ `could not read Username for 'https://github.com': Device not configured`
- เมื่อเจออาการข้างบน ห้ามวน retry, ห้าม `unset GH_TOKEN`, ห้ามขอ escalation, ห้ามเสนอ `gh auth login --insecure-storage` หรือใส่ token ใน env; ให้หยุดที่ commit บน feature branch แล้วรายงาน handoff: ชื่อ branch, จำนวน commit, base branch และคำสั่ง `git push -u origin <branch>` + `gh pr create` ให้ user หรือ Claude Code (ซึ่งรัน `gh` นอก sandbox ผ่าน `excludedCommands`) เป็นคนส่ง
