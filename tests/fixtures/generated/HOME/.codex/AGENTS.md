...
- เอกสารที่สร้างใหม่ (spec, requirements, design, tasks, plan, report, handoff, retro) เขียน prose เป็นภาษาไทย รวม requirement ในรูป "เมื่อ<เหตุการณ์> ระบบต้อง<พฤติกรรม>"; คง REQ-ID/F-ID/B-ID, `Status:` พร้อมค่า enum เดิม, schema key, checkbox syntax และ heading/คอลัมน์ที่เครื่องมืออ่าน; `Evidence:` เป็น block แยกและระบุเฉพาะคำสั่งกับผลที่รันจริง; ไม่แปลเอกสารเก่าย้อนหลัง
...
- agent ห้ามอ่าน แสดง คัดลอก หรือแก้ `~/.codex/gh` และ `~/.claude/gh` (hook DENY `CREDENTIAL_READ`); ห้าม `gh auth token`; ห้ามใส่ token ใน env หรือ command
- ถ้า `gh` ตอบ `HTTP 401` / `Bad credentials` / `gh auth login` หรือ `git push` ตาย `could not read Username` แปลว่า agent token ยังไม่ได้ตั้ง หมดอายุ หรือถูกย้ายเข้า keyring (user ต้องรัน `agents-adapter doctor` ดู `gh agent token storage (codex)`); 401 จาก `gh` ไม่ได้บล็อกงาน local: ทำ commit/test/spec ให้ครบแล้วรายงาน; ถ้า push โดน `refusing to allow an OAuth App to create or update workflow ... without \`workflow\` scope` แปลว่า token ไม่มี scope `workflow`: ทั้งสองกรณีห้ามวน retry ห้าม `gh auth login`/`gh auth refresh` เอง ห้ามแยก commit เพื่อเลี่ยง `.github/workflows` ให้ commit บน feature branch แล้วรายงาน user ให้รัน setup ใน docs/codex-adapter.md (GitHub) หรือ push จาก Claude Code/terminal
- `dotnet test` ทำงานใน sandbox ได้ (env `DOTNET_SYSTEM_NET_DISABLEIPV6=1` ถูกตั้งให้ทุก shell command เพราะ seatbelt ปฏิเสธ v4-mapped IPv6 loopback ที่ testhost ใช้); ถ้ายังเจอ `failed to connect to testhost` แปลว่า session เริ่มก่อน apply ให้ restart codex ห้ามวน retry/แก้ timeout
- `docker build --pull` ทำงานใน sandbox ได้ (วัดด้วย `codex sandbox` บน profile นี้: docker.sock ใน `unix_sockets`, `~/.docker/buildx` เป็น write); `~/.docker/config.json` ยัง deny โดยตั้งใจ ถ้าเจอ `pull access denied`/`unauthorized` กับ image ใน registry ที่ต้อง login ให้ user `docker pull` เอง; command ที่ติด sandbox ด้วยเหตุอื่นยังใช้ `sandbox_permissions: "require_escalated"` ได้ตาม rule
- process: seatbelt ของ Codex ปฏิเสธ `ps`/`top` (setuid, `execvp() ... Operation not permitted`), `pgrep`/`pkill` (`sysmond service not found`, exit 3 = อ่านไม่ได้ ไม่ใช่ไม่พบ) และ signal ไป process ของ user นอก sandbox (`kill: (<pid>) - Operation not permitted`) ไม่มี config key แก้; หา listener ด้วย `lsof -nP -iTCP:<port> -sTCP:LISTEN`; port ที่ dev/test server ของ repo ปัจจุบันค้างยึดให้รัน `${HOME}/.codex/hooks/agents-adapter/agents-free-port.sh <port>` ด้วย `sandbox_permissions: "require_escalated"` จาก cwd ใน git work tree ของ repo นั้น (kill เฉพาะ listener ที่ cwd อยู่ใน repo นี้ ปฏิเสธ port ต่ำกว่า 1024 และ cwd ที่ไม่ใช่ git work tree) ถ้าตอบ `refused: running inside the sandbox` (exit 4) แปลว่าไม่ได้ escalate ให้เรียกใหม่ด้วย `require_escalated` ถ้าตอบ `refused: pid ... outside` (exit 3) หรือ `failed:` ให้ user หยุด process เอง
- ตรวจ capability ของ sandbox ทั้งชุดด้วย `bash ${HOME}/.codex/hooks/agents-adapter/sandbox-probe.sh` (PASS / DENY(known) / FAIL / SKIP; pgrep และ signal เป็น DENY(known) บน Codex) ก่อนสรุปว่าอาการเกิดจาก sandbox
...
ค่าเริ่มต้น: เมื่อ user มอบหมาย objective ให้สำรวจ repo เลือกแนวทางที่ปลอดภัยและย้อนกลับได้ บันทึก assumptions แล้วทำต่อจนผ่าน review และ test gates ใน scope ที่ได้รับมอบหมาย ใช้ spec ที่มีเป็นข้อกำหนดและรักษา traceability; ไม่เรียก interactive workflow หรือสร้างขั้นรออนุมัติเองโดยอัตโนมัติ

เลือก interactive workflow ที่หยุดรอ phase approval เฉพาะเมื่อ user ขอ review/sign-off แต่ละ phase อย่างชัดเจน การเรียก slash command หรือ skill ของ spec workflow อย่างเดียวไม่ถือเป็นคำขอให้หยุดรอทุก phase ให้ทำเฉพาะผลลัพธ์ที่คำสั่งนั้นขอ คำสั่ง STOP/Wait/Pause ใน workflow หรือ skill ของ repository ใช้เฉพาะ interactive workflow นี้; review คุณภาพ, test gates และข้อกำหนดของ spec ยังใช้เสมอ

ถามเฉพาะ product decision สำคัญที่อนุมานจาก repo หรือ objective ไม่ได้ หรือ action ที่ต้องมี external/destructive authorization และยังไม่ได้รับ; ทำส่วนที่เป็นอิสระต่อระหว่างรอ คงข้อห้าม secrets, production, protected refs และ permission hooks ทั้งหมด บันทึก `Status: approved` เฉพาะเมื่อ user อนุมัติจริง; หาก hook บังคับ approval ให้รายงาน blocker และทำส่วนที่ไม่ติด gate ต่อ ห้ามสร้าง approval metadata หรือเลี่ยง hook เพื่อให้ผ่าน
