...
| Codex: `gh` หรือ `git push` ตาย `failed to load config: open ~/.config/gh/config.yml: operation not permitted` / `could not read Username` | permission profile ที่ติดตั้งยัง deny `~/.config/gh`; deny entry เป็น escalatable=false จึงแก้ด้วย `require_escalated` ไม่ได้ | `git pull` แล้ว `apply --target codex` (profile ใหม่ให้ `~/.config/gh` = read); agent ยังห้ามอ่านไฟล์ในนั้นเอง (hook DENY) |
