...
| Codex: `gh` ตาย `failed to load config: open ~/.config/gh/config.yml: operation not permitted` | permission profile deny `~/.config/gh` (ตั้งใจ) แต่ command ถูกรันใน sandbox | เรียก shell ด้วย `sandbox_permissions: "require_escalated"`; allow rule `["gh"]`, `git push/pull/ls-remote/clone`, `docker`, `dotnet test` ทำให้ไม่ prompt (ต้อง `apply --target codex` ให้ได้ rules ใหม่); forbidden/prompt rule ที่เจาะจงกว่ายังชนะ |
