...
| Claude: `ps` ตอบ `operation not permitted: /bin/ps` (exit 127) และ `pgrep`/`pkill` ตอบ `sysmond service not found` + `Cannot get process list` | seatbelt ปฏิเสธการ enumerate process ทั้งหมด ไม่ใช่ rtk (probe: `/bin/ps` ตรง ๆ และ `rtk proxy ps` ล้มเหมือนกัน); `pgrep` คืน exit 0 พร้อมผลว่าง จึงอ่านเป็น "ไม่มี process นี้" ทั้งที่อ่านไม่ได้ | เช็ค service ที่ฟัง port ด้วย `lsof -nP -iTCP -sTCP:LISTEN` (ทำงานใน sandbox ได้); ห้ามสรุปจาก `pgrep` ว่าไม่มี process; ถ้าต้องดู process list จริงให้ user รันเองด้วย `!` prefix — ไม่เพิ่ม `ps *` ใน `excluded_commands` เพราะ segment เดียวที่ match ยกทั้งบรรทัดออกนอก sandbox |
