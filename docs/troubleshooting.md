...
| Codex/Pi: DENY `PROD_ENV_READ` บน `.env.prod.example` | pattern `.env.prod.*` จับ template ค่าปลอมด้วย | `git pull` แล้ว `apply`; ตอนนี้ `.example`/`.sample`/`.dist`/`.template` ไม่นับเป็น env จริง; Claude ยัง deny เพราะ glob ใน `permissions.deny` ไม่มี negation ให้ user อ่านเองหรือ `git show HEAD:.env.prod.example` |
