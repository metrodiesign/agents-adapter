...
| Codex: `gh` ตอบ `HTTP 401: Requires authentication` / `gh auth status` ใน sandbox ตอบ `The token in default is invalid` / `git push` ตาย `could not read Username for 'https://github.com': Device not configured` ทั้งที่นอก Codex ใช้ได้ | token อยู่ใน macOS keychain; seatbelt ของ Codex ปิด keychain ทั้งใน sandbox และ escalated (`codex sandbox --log-denials -- gh api user` แสดง `file-read-metadata .../login.keychain-db`) ไม่ใช่ `GH_TOKEN` stale และไม่ใช่ hook | ไม่มี config ของ agents-adapter แก้ได้โดยไม่เปิด credential store: ให้ Codex หยุดที่ commit แล้ว push/เปิด PR จาก Claude Code หรือ terminal (AGENTS.md managed block สั่งไว้แล้วหลัง `apply --target codex`) |
