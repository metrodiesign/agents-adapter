...
| Claude: `ps` ตอบ `operation not permitted: /bin/ps` (exit 127) และ `pgrep`/`pkill` ตอบ `sysmond service not found` + `Cannot get process list` | seatbelt ปฏิเสธทั้งการ enumerate process และการส่ง signal ไป process นอก tree ของ sandbox เอง (`kill -TERM -<pgid>` ตอบ `operation not permitted`) ไม่ใช่ rtk (probe: `/bin/ps` ตรง ๆ และ `rtk proxy ps` ล้มเหมือนกัน); `pgrep`/`pkill` ล้มดัง ๆ ด้วย exit 3 ไม่ใช่คืนผลว่างเงียบ ๆ | เช็ค service ที่ฟัง port ด้วย `lsof -nP -iTCP -sTCP:LISTEN` (ทำงานใน sandbox ได้); ห้ามสรุปว่า process ไม่มีอยู่เมื่อ `pgrep` ตอบ exit 3 (นั่นคืออ่านไม่ได้ ไม่ใช่ไม่พบ); ถ้าต้องดู process list จริงให้ user รันเองด้วย `!` prefix — ไม่เพิ่ม `ps *` ใน `excluded_commands` เพราะ segment เดียวที่ match ยกทั้งบรรทัดออกนอก sandbox |
