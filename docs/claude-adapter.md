...
| `sandbox.enabled`, `sandbox.failIfUnavailable` | ตาม user config `sandbox.enabled` (default `true`); `false` = ปิด OS sandbox ทั้งตัว (permission rules, classifier และ hook ยังบังคับ) `doctor` รายงาน `claude sandbox` เป็น PASS เมื่อไฟล์ตรงกับ config |
| `sandbox.autoAllowBashIfSandboxed`, `sandbox.allowUnsandboxedCommands`, `sandbox.network.allowLocalBinding` | บังคับ `true`; ถ้า `allowUnsandboxedCommands` เป็น false ทั้ง `excludedCommands` จะไร้ผลโดยไม่มีสัญญาณเตือน |
